...
public enum BackupService {
    ...
    // MARK: — v1.12 Export Markdown human-readable

    /// Génère un dump Markdown des memories + audits + projects + signals récents.
    /// Plus consommable par Mehdi en plain text que le JSON technique.
    @MainActor
    public static func exportAsMarkdown(container: ModelContainer, to dir: URL? = nil) throws -> URL {
        let context = container.mainContext
        let now = Date()
        let isoFormatter = ISO8601DateFormatter()

        let memories = (try? context.fetch(FetchDescriptor<Memory>(sortBy: [SortDescriptor(\.createdAt, order: .reverse)]))) ?? []
        let audits = (try? context.fetch(FetchDescriptor<AuditReport>(sortBy: [SortDescriptor(\.createdAt, order: .reverse)]))) ?? []
        let projects = (try? context.fetch(FetchDescriptor<ProjectRecord>(sortBy: [SortDescriptor(\.lastPushAt, order: .reverse)]))) ?? []
        let oneDayAgo = now.addingTimeInterval(-86400)
        let recentSignals = (try? context.fetch(FetchDescriptor<Signal>(
            predicate: #Predicate { $0.emittedAt >= oneDayAgo },
            sortBy: [SortDescriptor(\.emittedAt, order: .reverse)]
        ))) ?? []

        var md = ""
        md += "# IRIS export — \(isoFormatter.string(from: now))\n\n"
        md += "Generated by IRIS v\(backupVersion) · backup human-readable.\n\n"

        // Memories grouped by type
        md += "## Memories (\(memories.count))\n\n"
        let memByType = Dictionary(grouping: memories, by: \.type)
        for typeKey in ["user", "feedback", "project", "reference", "conversation"] {
            guard let mems = memByType[typeKey], !mems.isEmpty else { continue }
            md += "### \(typeKey) (\(mems.count))\n\n"
            for mem in mems {
                let scope = mem.projectScope.map { " *[\($0)]*" } ?? ""
                md += "- **\(mem.name)**\(scope) — \(mem.summary.isEmpty ? "(no summary)" : mem.summary)\n"
            }
            md += "\n"
        }

        // Audits
        md += "## Audits récents (\(audits.count))\n\n"
        for audit in audits.prefix(20) {
            md += "### \(audit.projectCodename) — **\(audit.verdict)** (\(isoFormatter.string(from: audit.createdAt)))\n\n"
            md += "\(audit.headline)\n\n"
            if let findingsData = audit.findingsJSON.data(using: .utf8),
               let findings = try? JSONSerialization.jsonObject(with: findingsData) as? [String],
               !findings.isEmpty {
                md += "**Findings** :\n"
                for f in findings {
                    md += "- \(f)\n"
            ...
            md += "Model : `\(audit.modelUsed)` · Cost : $\(String(format: "%.5f", audit.costUSD)) · Duration : \(String(format: "%.1f", audit.durationSeconds))s\n\n"
        }

        // Projects table
        md += "## Projets (\(projects.count))\n\n"
        md += "| codename | status | repo | local |\n|---|---|---|---|\n"
        for project in projects.prefix(50) {
            let repo = project.repoURL.map { "[\($0.replacingOccurrences(of: "https://github.com/", with: ""))](\($0))" } ?? "—"
            let local = project.localPath.map { "`\($0.replacingOccurrences(of: NSHomeDirectory(), with: "~"))`" } ?? "—"
            md += "| `\(project.codename)` | \(project.status) | \(repo) | \(local) |\n"
        }
        md += "\n"

        // Recent signals 24h
        md += "## Signaux dernières 24h (\(recentSignals.count))\n\n"
        for signal in recentSignals.prefix(50) {
            let imp = String(repeating: "🟢", count: max(0, signal.importance))
            md += "- \(imp) [\(signal.source)] \(isoFormatter.string(from: signal.emittedAt)) — \(signal.summary)\n"
        }
        md += "\n"
        ...
        let targetDir = dir ?? URL(fileURLWithPath: NSHomeDirectory())
        let url = targetDir.appendingPathComponent("iris-export-\(dateStr).md")
